...
#   } | local/join_suffix.py > data/$set.orig/stm
  ...
  if [ ! -e $dir/source.txt ]
  then
    find ${mybuddy_db}/${mybuddy_collection_name}/data/$set/ -name "*.trn" -exec sh -c \
      'NAME=`basename {} .trn`; DIR=`dirname {}`; FLAC="${DIR}/${NAME}.flac"; TEXT=`cat {} | tr "\n" " "` ; if [ -e ${FLAC} ] ; then echo "${FLAC} $TEXT" ; fi ' \; \
      > $dir/source.txt
  fi

  local/prepare_data_text.pl < $dir/source.txt > $dir/text
  local/prepare_data_wav.pl < $dir/source.txt > $dir/wav.scp
  local/prepare_data_utt2spk.pl < $dir/source.txt > $dir/utt2spk
